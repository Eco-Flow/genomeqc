...
include { TIDK_FIND                                 } from '../../../modules/local/tidk/find'
...
include { TIDK_PLOT as TIDK_FIND_PLOT               } from '../../../modules/nf-core/tidk/plot'
...
workflow FASTA_EXPLORE_SEARCH_PLOT_TIDK {
    ...
    ch_apriori_sequence     // channel: [ val(meta), val(sequence) ]; Optional: Set to [] if not needed
                            ...
                            // the same `id`
    ch_clade                // channel: [ val(clade) ]; Optional: Set to [] if not needed
    ...
    // TIDK_FIND
    TIDK_FIND ( ch_sorted_fasta, ch_clade ? ch_clade : channel.empty() )

    ch_tidk_find_tsv        = TIDK_FIND.out.tsv
    ...
    // TIDK_PLOT as TIDK_FIND_PLOT
    TIDK_FIND_PLOT ( ch_tidk_find_tsv )

    ch_find_svg             = TIDK_FIND_PLOT.out.svg
    ...
    find_tsv                 = ch_tidk_find_tsv     // channel: [ val(meta), tsv ]
    find_svg                 = ch_find_svg          // channel: [ val(meta), svg ]
}
